...
class SongServiceEditorModel: ObservableObject {
    ...
    @ViewBuilder var doneButton: some View {
        Button {
            if viewModel.customSelectedSongs.count > 0 {
                songService.item.set(sectionedSongs: [SongServiceSectionWithSongs(title: "", cocList: viewModel.customSelectedSongs.map { .cluster($0) })])
            } else {
                songService.item.set(sectionedSongs: viewModel.sectionedSongs)
            }
